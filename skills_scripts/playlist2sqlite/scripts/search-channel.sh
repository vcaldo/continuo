...
JSON_OUTPUT=false
M3U_OUTPUT=""  # M3U output file path
...
Options:
  ...
  --m3u FILE         Also generate M3U playlist with search results
...
Examples:
  ...
  # Generate M3U playlist with results
  ./search-channel.sh --db channels.db --query "BBB" --m3u bbb-channels.m3u
...
# Generate M3U playlist from results
generate_m3u() {
    local output_file="$1"
    local query="$2"
    shift 2
    ...
    if [[ ${#results[@]} -eq 0 ]]; then
        warn "No results to write to M3U file"
        return 1
    fi
    
    log "Generating M3U playlist: $output_file"
    
    {
        echo "#EXTM3U"
        echo "# Generated by playlist2sqlite search-channel.sh"
        echo "# Query: $query"
        ...
        for result in "${results[@]}"; do
            ...
            local extinf="#EXTINF:-1"
            
            # Add tvg-name
            if [[ -n "$name" ]]; then
                extinf="$extinf tvg-name=\"$name\""
            fi
            
            # Add group-title
            if [[ -n "$group" ]]; then
                extinf="$extinf group-title=\"$group\""
            fi
    ...
    log "M3U playlist saved: $output_file (${#results[@]} channels)"
    
    if [[ "$VERBOSE" != true ]]; then
        echo ""
        echo -e "${GREEN}✓${NC} M3U playlist created: $output_file (${#results[@]} channels)"
    fi
}
...
main() {
    ...
    if [[ -n "$M3U_OUTPUT" ]]; then
        generate_m3u "$M3U_OUTPUT" "$QUERY" "${results[@]}"
    fi
}
